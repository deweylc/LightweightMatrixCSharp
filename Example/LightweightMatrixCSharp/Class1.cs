...
namespace LightweightMatrixCSharp
{
    public class Matrix
    {
        ...
        public Matrix(int iRows, int iCols)         // Matrix Class constructor  //初始化矩阵
        {
            rows = iRows;
            cols = iCols;
        ...
        public Matrix(double[,] matrix, bool rowMajor = true)//根据已有矩阵创建新矩阵？【复制用？存疑
        {
            if (rowMajor)
            {
        ...
        public Boolean IsSquare()//判断是否为方阵
        {
            return (rows == cols);
        }

        public double this[int iRow, int iCol]      // Access this matrix as a 2D array //对矩阵某个元素进行读写
        {
            get { return mat[iRow * cols + iCol]; }
            set { mat[iRow * cols + iCol] = value; }
        }

        public Matrix GetCol(int k)//获取k列向量
        {
        ...
        public void SetCol(Matrix v, int k)//设置k列向量
        {
        ...
        public override string ToString()                           // Function returns matrix as a string
        {
            ...
            {
                for (int j = 0; j < cols; j++)
                    s.Append(String.Format("{0,-5}", this[i, j]) + " ");
                s.AppendLine();
            }
            return s.ToString();
        }
        ...
        private static Matrix Multiply(double n, Matrix m)                          // Multiplication by constant n //矩阵数乘
        {
        ...
        private static Matrix Add(Matrix m1, Matrix m2)         // Sčítání matic  //矩阵加法
        {
